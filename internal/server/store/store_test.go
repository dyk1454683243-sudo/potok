...
func TestCreateVaultDuplicateNameDifferentCase(t *testing.T) {
	...
	got, err := s.VaultByName(context.Background(), user.ID, "notes")
	if err != nil {
		t.Fatalf("VaultByName() after rejected duplicates: %v", err)
	}
	if got.Name != "Notes" {
		t.Errorf("stored Name = %q, want original casing %q", got.Name, "Notes")
	}
	if got.ID != created.ID {
		t.Errorf("ID = %q, want the original vault %q", got.ID, created.ID)
	}
}
...
func TestVaultByNameCaseInsensitive(t *testing.T) {
	...
	for _, name := range []string{"Notes", "notes", "NOTES", "nOtEs"} {
		...
		if !bytes.Equal(got.KDFSalt, created.KDFSalt) {
			t.Errorf("VaultByName(%q) KDFSalt = %x, want %x", name, got.KDFSalt, created.KDFSalt)
		}
	}
}
